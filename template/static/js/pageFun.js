...
function playUser(e,userName) {
    inputName = 'input-count-'+userName;
    let count = parseInt($("input[name='"+inputName+"']").val());

    if (isNaN(count) || count == 0){ alert("请输入数量！"); return }
...
function playAll(e) {
    ...
    if (isNaN(count) || count == 0){ alert("请输入人数！"); return }

    let status = true;
    if (e.text == "PLAY ALL"){ e.text = "STOP";

    } else { e.text = "PLAY ALL"; status = false }
    var jsonStr = JSON.stringify({
        allCount:count,
        status:status,
        serverHost:getCookie("serHost"),
...
function loadUserList() {
    $.post("/userList",{},function(result){
        ...
        if (result.data != null && _list.length > 0){
                ...
                if ($("#tr-"+_list[i].Account).length > 0) {
                    ...
                    strHtml +=    `<tr id="tr-`+_list[i].Account+`">
                                    <td><a class="btn btn-primary btn-sm glyphicon glyphicon-search" href="/user?account=`+_list[i].Account+ `" role="button" title="查看"></a>
                                        ...
                                        <input type="text" class="form-control" name="input-count-`+ _list[i].Account +`" placeholder="人数" style="width:120px;display:unset;" aria-describedby="basic-addon1" value="1">
                                    </td>
            ...
            //theadTemp
            if (strHtml.length > 0){ $(".table").append(strHtml); }
